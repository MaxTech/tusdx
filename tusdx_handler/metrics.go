...
func (m Metrics) incErrorsTotal(err tusd.HTTPError) {
...
func simplifyHTTPError(err tusd.HTTPError) simpleHTTPError {
...
func (e *ErrorsTotalMap) retrievePointerFor(err tusd.HTTPError) *uint64 {
...
func (e *ErrorsTotalMap) Load() map[tusd.HTTPError]*uint64 {
    m := make(map[tusd.HTTPError]*uint64, len(e.counter))
    e.lock.RLock()
    for err, ptr := range e.counter {
        httpErr := tusd.NewHTTPError(errors.New(err.Message), err.StatusCode)
        m[httpErr] = ptr
    }
    e.lock.RUnlock()
